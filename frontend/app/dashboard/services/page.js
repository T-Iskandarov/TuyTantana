  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5 shadow-sm">
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full py-3 rounded-xl text-sm font-semibold bg-gradient-to-r from-[#7C3AED] to-[#6D28D9] text-white hover:shadow-lg hover:shadow-[#7C3AED]/20 transition-all disabled:opacity-50"
      >
        {loading ? 'Saqlanmoqda...' : 'Xizmatni saqlash'}
      </button>
    </form>
  );
}
